...
%This m_file seperates particles accor ding to their ID in the 'result_tracking', and
...
% Finding clusters
[Class,type]=dbscan_conservative(data_Syn,100,250); 
% Make new matrix  
...
if optional
    ...
    for k=1:nSyn2
        ...
        TS_X=trace_n_syn1{syn_nearby0(k)}(:,1)-Syn_ctrX; % Trace to synapse distance delta X
        TS_Y=trace_n_syn1{syn_nearby0(k)}(:,2)-Syn_ctrY; 
        TS_Z=trace_n_syn1{syn_nearby0(k)}(:,3)-Syn_ctrZ;
        TS_dist{syn_nearby0(k)}=sqrt(TS_X.*TS_X+TS_Y.*TS_Y+TS_Z.*TS_Z)/1000; % Distance of each point on trace to the center of synapse in um
    end
    
    save(strcat(userdirin,'trace_n_syn1.mat'),'trace_n_syn1');% Add by Anne 11/26/2013
    save(strcat(userdirin,'TS_dist.mat'),'TS_dist');% Add by Anne 11/26/2013
...
figure; hist(cell2mat(TS_dist'),48) % add by Anne 11/26/2013
