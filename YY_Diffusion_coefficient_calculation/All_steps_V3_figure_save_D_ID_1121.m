...
path = 'G:\Data\BrainSlices\GluR2_antibody\180914_newantiGluA2-QD_specificity_neuron_HeLa\shortPreincubation_GluR2-ab_testing\2-2';
...
if optional
    ...
    for rg=1:nSyn
        ...
        pID=syn_nearby{rg};
        syn_nearby_d0=syn_nearby{rg};
        syn_nearby_d0(:,2)=syn_traces(pID,4);
        syn_nearby_d{rg}=syn_nearby_d0;
    ...
    save(strcat(userdirin,'syn_nearby_d.mat'),'syn_nearby_d');
...
%     Sig0 = (radii .* (max(syn_rg) - min(syn_rg))) / sqrt(8.0);
    scatter3(syn_rg(:,1), syn_rg(:,2), syn_rg(:,3),30,'b'); % Plot scattered data in original space
    ...
    Syn_buff=250; % The buffer layer outside synapse
...
clear x y z frame particle k d m x1 y1 z1 frame1 particle1 dp ID_length k1

%%%%%%%%%%%%%%%%
%%%%%Calculate diffusion coefficients IN and OUT of the synapses

%% New version of IN and OUT diffusion coefficient
mTrace_INs=sortrows(mTraceIN,4);
mTrace_OUTs=sortrows(mTraceOUT,4);
...
zi=mTrace_INs(:,3);
ti=mTrace_INs(:,4);
positionlisti=[xi yi zi ti];

xo=mTrace_OUTs(:,1);
yo=mTrace_OUTs(:,2);
zo=mTrace_OUTs(:,3);
to=mTrace_OUTs(:,4);  
positionlisto=[xo yo zo to];
...
result_tracking_IN = track( positionlisti, maxdisp, param );
...
result_tracking_OUT = track( positionlisto, maxdisp, param );
clear param macdisp

fid=fopen([userdirin 'result_tracking_IN.txt'],'w');
fprintf(fid,'%f %f %f %d %d\n',result_tracking_IN');    
fclose(fid);
fid=fopen([userdirin 'result_tracking_OUT.txt'],'w');
fprintf(fid,'%f %f %f %d %d\n',result_tracking_OUT');    
fclose(fid);
...
tic
clear x1 y1 z1 frame1 particle1 d1 ptotal1 tSteps Dif1
x1=result_tracking_IN(:,1);
y1=result_tracking_IN(:,2);
z1=result_tracking_IN(:,3); % 0.79 is the correction factor for RI mismatch for water/oil for an 1.40 NA objective
frame1=result_tracking_IN(:,4);
particle1=result_tracking_IN(:,5);
d1=length(particle1); %total row number
ptotal1=particle1(d1);%total particle number
tStepsi=100;
clear Dif
clear MSD_Ind
% figure
% hold all
for j1=1:ptotal1
    dp1=find(particle1==j1); %dimension of matrix of particle# j
    dplength1=length(dp1);
    dpmax1=max(dp1); % for particlej, the first row number
    dpmin1=min(dp1); % for particlej, the last row number
    m(j1)=j1; %test j value
    x11=x1(dpmin1:dpmax1);
    y11=y1(dpmin1:dpmax1);
    z11=z1(dpmin1:dpmax1);
    frame11=frame1(dpmin1:dpmax1);
    particle11=particle1(dpmin1:dpmax1); % Added by Anne 1/14/2014
    TraceAll{j1}=[x11,y11,z11,frame11,particle11]; % Modified by Anne 1/14/2014
    Trace1_XYZ=[x11,y11,z11];
    [MSD001,d2r01,counts1,E_sem1]=fMSD_vect_withError(x11,y11,z11,frame11,dpmax1,dpmin1,tStepsi); % Call function MSD, MSD.m file must be in the same folder as this file
    cutoff=5;  %cutoff for data points
    ind1=find(counts1>cutoff-1); % find index of counts that is equate to and above cutoff
    MSDCF1=MSD001(ind1); % find the MSD for those index from last line
    ind1_log=log10(ind1*dt);
    MSDCF1_log=log10(MSDCF1*10^(-6));
    Err_sem1=E_sem1(ind1);
%     figure
%     subplot(2,1,1)
%     plot(ind,MSDCF)
...
%     plot(ind_log,MSDCF_log)
    indlength1=length(ind1);
    if indlength1>=10
        ind11=ind1_log(1:10);
        MSDCF11=MSDCF1_log(1:10);
        subDif_IN(j1,:)= polyfit(ind11',MSDCF11',1);
        
% ORIGINAL        f = fit(ind1',MSDCF1','poly1');
% ORIGINAL       Dif(j,:)=coeffvalues(f); % First column is the diffusion coeff (slope). To get um^2/s multiply by 2*10^(-5), to get cm^2/s, multiply by 2*10^(-13)
    else subDif_IN(j1,:)=[0,0]; ind11=ind1; MSDCF11=MSDCF1; %%% Changed on 2013/05/13, Dif has to have 2 columns, and if indlenth<4, ind1 and MSDCF1 has to have values
    end
    particle2(j1)=j1; % Added by Anne, 1/14/2014
    %plot(ind11,MSDCF11)
    
     if indlength1>=4
        ind11=ind1(1:4);
        MSDCF11=MSDCF1(1:4);
        Dif1(j1,:)= polyfit(ind11',MSDCF11',1);
        
% ORIGINAL        f = fit(ind1',MSDCF1','poly1');
% ORIGINAL       Dif(j,:)=coeffvalues(f); % First column is the diffusion coeff (slope). To get um^2/s multiply by 2*10^(-5), to get cm^2/s, multiply by 2*10^(-13)
    else Dif1(j1,:)=[0,0]; ind11=ind1; MSDCF11=MSDCF1; %%% Changed on 2013/05/13, Dif has to have 2 columns, and if indlenth<4, ind1 and MSDCF1 has to have values
    end
...
% dt=0.05; %Default time step 0.05s=50ms
Dif_IN=Dif1(:,1);
Dif_positive_IN=Dif_IN(Dif_IN>0);
Dif_time_IN=Dif_positive_IN/(dt*2*3); %D for time step dt, for 3D
Dif_track_IN=Dif_positive_IN/(dt*2*3*10^6);
% Dif_track_ID=particle2(Dif1>0); % Added by Anne, 1/14/2014, for trace ID of corresponding D
% Dif_track_W_ID(:,1)=Dif_track;
% Dif_track_W_ID(:,2)=Dif_track_ID;
save(strcat(userdirin,'diffusion_coefficients_IN.mat'),'Dif_track_IN');
figure
hist(Dif_track_IN, 30);
saveas(gcf,strcat(userdirin,'Dif_track_IN'),'fig')
subDif1_IN(:,1)=nonzeros(subDif_IN(:,1));
subDif1_IN(:,2)=nonzeros(subDif_IN(:,2));
save(strcat(userdirin,'subDif_IN.mat'),'subDif1_IN');
% save(strcat(userdirin,'diffusion_coefficients.mat'),'Dif_track_W_ID');
% save(strcat(userdirin,'Trace_range.mat'),'Trace_range')
% clear x y z frame particle j d m x1 y1 z1 frame1 dp
%% OUT
tic
clear x2 y2 z2 frame2 particle2 d2 ptotal2 tStepso
x2=result_tracking_OUT(:,1);
y2=result_tracking_OUT(:,2);
z2=result_tracking_OUT(:,3); % 0.79 is the correction factor for RI mismatch for water/oil for an 1.40 NA objective
frame2=result_tracking_OUT(:,4);
particle2=result_tracking_OUT(:,5);
d2=length(particle2); %total row number
ptotal2=particle2(d2);%total particle number
tStepso=100;
clear Dif
clear MSD_Ind
% figure
% hold all
for j2=1:ptotal2
    dp2=find(particle2==j2); %dimension of matrix of particle# j
    dplength2=length(dp2);
    dpmax2=max(dp2); % for particlej, the first row number
    dpmin2=min(dp2); % for particlej, the last row number
    m(j2)=j2; %test j value
    x22=x2(dpmin2:dpmax2);
    y22=y2(dpmin2:dpmax2);
    z22=z2(dpmin2:dpmax2);
    frame22=frame2(dpmin2:dpmax2);
    particle22=particle2(dpmin2:dpmax2); % Added by Anne 1/14/2014
    TraceAll{j2}=[x22,y22,z22,frame22,particle22]; % Modified by Anne 1/14/2014
    Trace2_XYZ=[x22,y22,z22];
    [MSD002,d2r02,counts2,E_sem2]=fMSD_vect_withError(x22,y22,z22,frame22,dpmax2,dpmin2,tStepso); % Call function MSD, MSD.m file must be in the same folder as this file
    cutoffo=20;  %cutoff for data points
    ind2=find(counts2>cutoffo-1); % find index of counts that is equate to and above cutoff
    MSDCF2=MSD002(ind2); % find the MSD for those index from last line
    ind2_log=log10(ind2*dt);
    MSDCF2_log=log10(MSDCF2*10^(-6));
    Err_sem2=E_sem2(ind2);
%figure
%     subplot(2,1,1)
%     plot(ind,MSDCF)
%     errorbar(MSDCF,Err_sem)
%     subplot(2,1,2)
%     plot(ind_log,MSDCF_log)
    indlength2=length(ind2);
    if indlength2>=20
        ind22=ind2_log(1:20);
        MSDCF22=MSDCF2_log(1:20);
        subDif_OUT(j2,:)= polyfit(ind22',MSDCF22',1);
        
    else subDif_OUT(j2,:)=[0,0]; ind22=ind2; MSDCF22=MSDCF2; %%% Changed on 2013/05/13, Dif has to have 2 columns, and if indlenth<4, ind1 and MSDCF1 has to have values
    end
    particle23(j2)=j2; % Added by Anne, 1/14/2014
    %plot(ind22,MSDCF22)
    
     if indlength2>=4
        ind22=ind2(1:4);
        MSDCF22=MSDCF2(1:4);
        Dif2(j2,:)= polyfit(ind22',MSDCF22',1);
        
    else Dif2(j2,:)=[0,0]; ind22=ind2; MSDCF22=MSDCF2; %%% Changed on 2013/05/13, Dif has to have 2 columns, and if indlenth<4, ind1 and MSDCF1 has to have values
    end
...
% dt=0.05; %Default time step 0.05s=50ms
Dif_OUT=Dif2(:,1);
Dif_positive_OUT=Dif_OUT(Dif_OUT>0);
Dif_time_OUT=Dif_positive_OUT/(dt*2*3); %D for time step dt, for 3D
Dif_track_OUT=Dif_positive_OUT/(dt*2*3*10^6);
save(strcat(userdirin,'diffusion_coefficients_OUT.mat'),'Dif_track_OUT');
% Dif_track_ID=particle2(Dif1>0); % Added by Anne, 1/14/2014, for trace ID of corresponding D
% Dif_track_W_ID(:,1)=Dif_track;
% Dif_track_W_ID(:,2)=Dif_track_ID;
figure
hist(Dif_track_OUT, 30);
saveas(gcf,strcat(userdirin,'Dif_track_OUT'),'fig')
subDif1_OUT(:,1)=nonzeros(subDif_OUT(:,1));
subDif1_OUT(:,2)=nonzeros(subDif_OUT(:,2));
save(strcat(userdirin,'subDif_OUT.mat'),'subDif1_OUT');
% save(strcat(userdirin,'diffusion_coefficients.mat'),'Dif_track_W_ID');
% save(strcat(userdirin,'Trace_range.mat'),'Trace_range')
% clear x y z frame particle j d m x1 y1 z1 frame1 dp
...
%%
% clear x_t y_t z_t t_t positionlist1 param maxdisp x y z frame particle d TraceIN ID_length tSteps 
% mTraceINs=sortrows(mTraceIN,4);
% x_t=mTraceINs(:,1);
% y_t=mTraceINs(:,2);
% z_t=mTraceINs(:,3);
% t_t=mTraceINs(:,4);
% positionlist1=[x_t y_t z_t t_t];
% param.mem=20;
% param.dim=3;
% param.good=10;
% param.quiet=0;
% maxdisp=1000;
% result_trackingIN = track( positionlist1, maxdisp, param );
% 
% x=result_trackingIN(:,1);
% y=result_trackingIN(:,2);
% z=result_trackingIN(:,3); % 0.79 is the correction factor for RI mismatch for water/oil for an 1.40 NA objective
% frame=result_trackingIN(:,4);
% particle=result_trackingIN(:,5);
% d=length(particle); % total row number
% TraceIN=unique(particle); % Original particle ID
...
%     plot(ind1,MSDCF1)
%     hold all
%     clear dp dplength dpmax dpmin x1 y1 z1 frame1 particle1 MSD00 d2r0 counts  MSDCF f MSDCF1 ind1 
% end
% 
% hold off
% dt=0.05; %Default time step 0.05s=50ms
% DifIN=Dif(:,1);
% Dif_positive_IN=DifIN(find(DifIN>0));
% Dif_time_IN=Dif_positive_IN/(dt*2*3); %D for time step dt, for 3D
% Dif_track_IN=Dif_positive_IN/(dt*2*3*10^6);
% figure
...
% 
% % OUT
% clear x_tO y_tO z_tO t_tO positionlist2 param maxdisp x_OUT y_OUT z_OUT frame_OUT particle_OUT d_OUT TraceOUT ID_length_OUT tSteps
% mTraceOUTs=sortrows(mTraceOUT,4);
% x_tO=mTraceOUTs(:,1);
% y_tO=mTraceOUTs(:,2);
% z_tO=mTraceOUTs(:,3);
% t_tO=mTraceOUTs(:,4);
% positionlist2=[x_tO y_tO z_tO t_tO];
% param.mem=20;
% param.dim=3;
% param.good=10;
% param.quiet=0;
% maxdisp=1000;
% result_trackingOUT = track( positionlist2, maxdisp, param );
% 
% x_OUT=result_trackingOUT(:,1);
% y_OUT=result_trackingOUT(:,2);
% z_OUT=result_trackingOUT(:,3); % 0.79 is the correction factor for RI mismatch for water/oil for an 1.40 NA objective
% frame_OUT=result_trackingOUT(:,4);
% particle_OUT=result_trackingOUT(:,5);
% d_OUT=length(particle_OUT); % total row number
% TraceOUT=unique(particle_OUT);
% ID_length_OUT=length(TraceOUT); % length of particle number
% tSteps=10;
% clear Dif
% %%MSD calculation
% figure
% for k1=1:ID_length_OUT
%     k=TraceOUT(k1);
%     dp=find(particle_OUT  ==k); % dimension of matrix of particle# K
%     dplength=length(dp);
%     if dplength<2
%         Dif(k,:)=[0, 0];
%     else
...
%         dpmin=min(dp); % for particleK, the last row number
%         m(k)=k; % test k value
%         x1=x_OUT(dpmin:dpmax);
%         y1=y_OUT(dpmin:dpmax);
%         z1=z_OUT(dpmin:dpmax);
%         frame1=frame_OUT(dpmin:dpmax);
%         particle1=particle_OUT(dpmin:dpmax);
%         Trace_IN{k}=[x1,y1,z1,frame1,particle1];
%         [MSD00,d2r0,counts]=fMSD_vect(x1,y1,z1,frame1,dpmax,dpmin,tSteps); % Call function MSD, MSD.m file must be in the same folder as this file
%         cutoff=10;  %cutoff for data points
%         ind=find(counts>cutoff-1); % find index of counts that is equate to and above cutoff
...
% dt=0.05; %Default time step 0.05s=50ms
% Dif_O=Dif(:,1);
% Dif_positive=Dif_O(find(Dif_O>0));
...
% clear x_tO y_tO z_tO t_tO positionlist2 param maxdisp x_OUT y_OUT z_OUT frame_OUT particle_OUT d_OUT TraceOUT ID_length_OUT tSteps
% save(strcat(userdirin,'diffusion_coefficients_in.mat'),'Dif_track_IN');
% old version of IN and OUT diffusion coefficient
% for k2=1:ID_length_IN
%     ki=TraceIN(k2);
%     dp_IN=find(particle==ki); % dimension of matrix of particle# K
%     dplength_IN=length(dp_IN);
%     if dplength_IN<2
%         Dif_IN(ki,:)=[0, 0];
%     else
%         dpmax_IN=max(dp_IN); % for particleK, the first row number
%         dpmin_IN=min(dp_IN); % for particleK, the last row number
%         m(ki)=ki; % test k value
%         x2=x_IN(dpmin_IN:dpmax_IN);
%         y2=y_IN(dpmin_IN:dpmax_IN);
%         z2=z_IN(dpmin_IN:dpmax_IN);
%         frame2=frame_IN(dpmin_IN:dpmax_IN);
%         particle2=particle_IN(dpmin_IN:dpmax_IN);
%         Trace_IN{ki}=[x2,y2,z2,frame2,particle2];
%         [MSD00_IN,d2r0_IN,counts_IN]=fMSD_vect(x2,y2,z2,frame2,dpmax_IN,dpmin_IN,tSteps); % Call function MSD, MSD.m file must be in the same folder as this file
%         cutoff=10;  %cutoff for data points
%         ind_IN=find(counts_IN>cutoff-1); % find index of counts that is equate to and above cutoff
%         MSDCF_IN=MSD00_IN(ind_IN); % find the MSD for those index from last line
%         %ind1=[0,ind]; % Add (0,0) as the first point of the curve
%         %MSDCF1=[0,MSDCF]; % Add (0,0) as the first point of the curve
%         % ind1=ind(1:4);
%         % MSDCF1=MSDCF(1:4);
%         indlength_IN=length(ind_IN);
%         if indlength_IN>=4
%             ind1_IN=ind_IN(1:4);
%             MSDCF1_IN=MSDCF_IN(1:4);
%             Dif_IN(ki,:) = polyfit(ind1_IN',MSDCF1_IN',1);
% % ORIGINAL            f = fit(ind1',MSDCF1','poly1');
...
%         else Dif_IN(ki,:)=[0,0]; ind1_IN=ind_IN; MSDCF1_IN=MSDCF_IN;
%         end
%     end
%     plot(ind1_IN,MSDCF1_IN)
%     hold all
%     clear dp_IN dplength_IN dpmax_IN dpmin_IN x2 y2 z2 frame2 particle2 MSD00_IN d2r0_IN counts_IN  MSDCF_IN f
% end
% hold off
% dt=0.05; %Default time step 0.05s=50ms
% Dif1_IN=Dif_IN(:,1);
% Dif_positive_IN=Dif1_IN(find(Dif1_IN>0));
% Dif_time_IN=Dif_positive_IN/(dt*2*3); %D for time step dt, for 3D
% Dif_track_IN=Dif_positive_IN/(dt*2*3*10^6);
% figure
% hist(Dif_track_IN, 15);
% clear x_IN y_IN z_IN frame_IN particle_IN k d_IN m x2 y2 z2 frame2 dp Dif1_IN Dif_positive_IN ID_length_IN
% save(strcat(userdirin,'diffusion_coefficients_in.mat'),'Dif_track_IN');
