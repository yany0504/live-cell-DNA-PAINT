...
path = 'F:\20210306_scFV_onTagBFP-hTau_wGluA1-AP_on_neurons\Live_neurons_w_GluA1AP_and_BirA\TagBFP-hTauWT_Homer-mGeos\110\';
outputpath='F:\20210306_scFV_onTagBFP-hTau_wGluA1-AP_on_neurons\Live_neurons_w_GluA1AP_and_BirA\TagBFP-hTauWT_Homer-mGeos\110\testResults\';
pathsplit=strsplit(path,'\');
outputindex=pathsplit(end-1);
outputname='mem3_good3_maxdisp500_difflength4'; % this will go to all the output filex e.g. result_tracking_picasso.txt
isitpicasso=0;
actualdimension=3;
% frame selection
...
%%log file generation
%%%%%%%%%%%%%%%%%%%%
logfileName = sprintf('log_%s.txt', datestr(now,'mm-dd-yyyy_HH-MM'));
fid=fopen([userdirin logfileName],'w');
fprintf(fid,'isitpicasso_%d\n',isitpicasso);
fprintf(fid,'timerange_%d\n',timerange);
fprintf(fid,'mint_%d\n',mint);
fprintf(fid,'maxt_%d\n',maxt);
fprintf(fid,'useROI_%d\n',useROI);
fprintf(fid,'xmin_%d\n',xmin);
fprintf(fid,'xmax_%d\n',xmax);
fprintf(fid,'ymin_%d\n',ymin);
fprintf(fid,'threshold_%d\n',threshold);
fprintf(fid,'factor_%d\n',factor);
fprintf(fid,'param.mem_%d\n',param.mem);
fprintf(fid,'actualdimension_%d\n',actualdimension);
fprintf(fid,'param.good_%d\n',param.good);
fprintf(fid,'param.quiet_%d\n',param.quiet);
fprintf(fid,'maxdisp_%d\n',maxdisp);
fprintf(fid,'tSteps_%d\n',tSteps);
fprintf(fid,'dt_%d\n',dt);
fprintf(fid,'k_%d\n',k);
fprintf(fid,'Eps_%d\n',Eps);
fprintf(fid,'filter_photon_%d\n',filter_photon);
fprintf(fid,'photon_cutoff_%d\n',photon_cutoff);
fclose(fid);
%%%%%%%%%%%%%%%%%%%%%%
...
traj_length=zeros(number_traj,1);

for k=1:number_traj;
   
    traj_length(k,:) = [trace_R(Dif_track_W_ID(k,2),6)];
   
end

output_diff_traj_dist=[Dif_track_W_ID(:,1) log10(Dif_track_W_ID(:,1)) traj_length/1000 log10(traj_length/1000) distance Dif_track_W_ID(:,2)]; % diff coeff, log(diff coeff), traj length, log(traj length), distance, ID 
xlswrite(strcat(outputpath,['DiffCoeff_TrajLength_Distance' [outputname '_' outputindex{1}] '.xlsx']), output_diff_traj_dist);
